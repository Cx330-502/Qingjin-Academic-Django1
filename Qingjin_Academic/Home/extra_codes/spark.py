...
NEW_CHAT = "SparkDesk AI chat ABAB"
...
class SparkWeb0:
    ...
    def chat_stream(self, history: bool = False, history_path: str = './history/'):
        ...
        if history:
            ...
            history_file_path = os.path.join(history_path,
                                             f'history_{str(datetime.now().strftime("%Y-%m-%d-%H-%M-%S"))}.json')
        try:
            self.__create_chat(NEW_CHAT)
    ...
    def rewrite_chat(self, history_path: str = './history/', history_file_path: str = "", question: str = ""):
        FileUtils.check_dir_path(history_path)
        ...
        try:
            self.__create_chat(NEW_CHAT)
            count = self.__chat_history.__len__() + 1
            if question == 'exit' or question == 'stop':
                return_data['answer'] = "退出成功"
                return return_data, history_file_path, self.__chat_id
            # resp = self.__streaming_output(question)
            response = self.__get_resp(question + "\n请不要使用Markdown语法回答 Please do not answer using Markdown syntax")
            response_text = ''
            for line in response.iter_lines():
                if line:
                    encoded_data = line[len("data:"):]
